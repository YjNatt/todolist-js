...
app.get("/lists/:todoListId",
  ...
  catchError(async (req, res) => {
    ...
    todoList.todos = await res.locals.store.sortTodos(todoList);
...
app.post("/lists/:todoListId/todos/:todoId/destroy",
  ...
  catchError(async (req, res, next) => {
  let { todoListId, todoId } = { ...req.params };
  let deleted = await res.locals.store.deletedTodo(+todoListId, +todoId);
  ...
  catchError(async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      res.render("list", {
        ...
        isDoneTodoList: res.locals.store.isDoneTodoList(todoList),
        hasUndoneTodos: res.locals.store.hsUndoneTodos(todoList),
      });
    } else {
...
app.post("/lists/:todoListId/destroy",
  ...
  catchError(async (req, res, next) => {
    let todoListId = req.params.todoListId;
    let deleted = await res.locals.store.deletedTodoList(+todoListId);
    if (!deleted) throw new Error('Not found.');
...
app.post("/lists",
  ...
  catchError(async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      if (await store.existsTodoListTitle(todoListTitle)){
        req.flash('error', 'Todo list title exists.');
        rerenderAddTodoList();
      } else {
